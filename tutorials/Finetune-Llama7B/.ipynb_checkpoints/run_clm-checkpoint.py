...
#        trainer.create_model_card(**kwargs)
    ...
    if (int(os.environ.get("RANK", -1)) == 0) and int(training_args.tensor_parallel_size) > 1:
        ...
        # new code to move the JSON file only if it exists(fix for a bug)
        # Author: vggangas@
        json_file_name = 'pytorch_model.bin.index.json'  # replace with the actual name of your JSON file
        source = os.path.join(os.getcwd(), json_file_name)
        destination = '/opt/ml/model/' + json_file_name
